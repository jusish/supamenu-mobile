    ...
    <ScrollView className="bg-white flex flex-1 flex-col px-8">
      {cart.length === 0 ? (
        <AnimationSection
          ...
          animation={require("@/assets/animations/food-cart.json")}
        />
      ) : (
        <View>
          <Text className="text-slate-500 mt-4">
            Please Continue to the checkout !
          </Text>
          <View>
            {cart.map((item) => (
              <View
              ...
              >
                <Image
                  className="h-12 w-12 border border-primary rounded-md"
                  source={{ uri: item.thumbnail }}
                />
                <View className="ml-4 w-60">
                  <Text className="font-semibold">{item.name}</Text>
                  <Text className="text-gray-400 text-justify pr-4">
          ...
          <View className="flex flex-row justify-between items-center my-6">
            <Text className="text-sm font-semibold">
              T.P : ${updateTotalPrice()}
            </Text>
            <Pressable
              onPress={() => router.push("/(home)/checkout")}
              disabled={updateTotalPrice() === 0}
              className={`bg-primary py-4 px-8 shadow-lg items-center rounded-md ${
                updateTotalPrice() === 0 ? "opacity-50" : "opacity-100"
              }`}
            >
